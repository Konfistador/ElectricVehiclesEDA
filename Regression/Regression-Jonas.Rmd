...
```{r}
  lmPrice <- lm(Range_Km ~ Battery_Pack_Kwh + I(Battery_Pack_Kwh^2) + I(Battery_Pack_Kwh^3) + PriceEuro, data = ev)
  summary(lmPrice)
```
  
```{r}
  ggplot(ev, aes(x=Battery_Pack_Kwh,y=PriceEuro)) + geom_point() + geom_smooth(method='auto',level=0.95)+ labs(x = "Capacity of the      Battery pack in KwH", y = "Price in Euro")
